...
Cowork が動いている PC のローカル Chrome が拡張アクティブな状態で起動していない。手順 5 のワンクリック復旧手段で起動。

### Yahoo が別 PC に開いてしまう（silent 誤接続）

スキルが Phase 1（list + select）をスキップした可能性。新規セッションで試す（スキルの description により自動発動するはず）。あるいはスキルが古いバージョン（v1）のままになっている可能性 → `.skill` を最新版で再インストール。

### Connect ボタンや名前入力ダイアログが出てくる

v2 スキルでは通常出ないはずですが、明示的に「switch_browser で選び直したい」と伝えた場合や、Phase 2A（リモート PC のみ候補のケース）に入った場合は出ます。名前入力は空欄のまま確定 OK（一度設定すれば以降は不要）。
