...
2. 初回のみターミナルで実行権限を付与（リポジトリから持ってきた場合、既に付与済みのはず）：
   ```bash
   chmod +x ~/Git/AgentSkills/cowork-chrome-launcher/scripts/open-cowork-chrome.command
   ```
...
ln -s AgentSkills/cowork-chrome-launcher cowork-chrome-launcher
```
...
git clone https://github.com/MasahikoShinya/AgentSkills.git
New-Item -ItemType SymbolicLink -Path "cowork-chrome-launcher" -Target "AgentSkills\cowork-chrome-launcher"
```
