...
test_workflow_resume_state() {
  ...
  repo="$(new_repo)"
  output="$(cd "$repo" && bash common/workflows/workflow-state.sh start sdd_tdd spec "進めてください" 2>&1)"
  assert_contains "$output" "started sdd_tdd" "Japanese continuation workflow state starts"
  output="$(cd "$repo" && bash common/workflows/workflow-state.sh show sdd_tdd "進めて" 2>&1)"
  assert_contains "$output" "Matched Japanese continuation shorthand" "Japanese continuation shorthand matches an unfinished workflow"
  assert_contains "$output" "resumable sdd_tdd workflow" "Japanese continuation shorthand resumes the workflow"
...
test_workflow_command_routes() {
  ...
  assert_contains "$publish_prompt" 'Both `::publish` and `::publish --loop` are complete invocations' "publish accepts bare normal and loop pseudo-commands"
  assert_contains "$publish_prompt" 'Do not require, suggest, or wait for added wording' "publish does not demand an execution phrase"
  ...
  assert_contains "$sdd_prompt" 'Treat `進めて`, `進めてください`, `続けて`, `続けてください`, `続行`, and `再開` as equivalent continuation signals' "SDD accepts Japanese continuation shorthand"
  assert_contains "$sdd_prompt" 'never ask the user to restate one of those phrases in a more polite form' "SDD does not require polite Japanese restatement"
  assert_contains "$sdd_prompt" 'If neither source is unique, report `PROMPT BLOCKER`' "SDD asks for a concrete target only when shorthand is ambiguous"
