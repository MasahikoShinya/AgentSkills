...
For `::sdd_tdd`, do not start tests until the adopted specification has been written to `SESSION_BRIEF.md`, and do not implement until failing-test or reproduction evidence has been obtained. Treat `進めて`, `進めてください`, `続けて`, `続けてください`, `続行`, and `再開` as equivalent continuation shorthand; do not ask the user to rewrite them politely. Resume one unfinished `sdd_tdd` workflow with its stored request, or adopt one applicable draft handoff with its stored request; ambiguity remains a `PROMPT BLOCKER`. For a new workflow, `::sdd_tdd <the exact request>` adopts exactly one matching draft `docs/plans/` handoff when present. Use its confirmed decisions as the Phase 1 input, validate only its named evidence and relevant changes since it was written, and do not repeat its broad survey. No match uses standalone Phase 1; multiple matches or unresolved decisions stop with `PROMPT BLOCKER`. `::sdd_tdd <request>` continues through Gate without phase-by-phase approval only when the request is confirmed and bounded; `::sdd_tdd --step <request>` executes one Phase. Stop for ambiguity, mixed existing changes, missing project-native test evidence, a final review `WARNING` / `BLOCKER`, a final `GATE` / `HOOK` `BLOCKER` / `FAIL`, or high-risk operations. An individual gate-check `WARNING` is informational when the final status is `PASS`.
...
For `::publish`, read `.agentskills/prompts/publish.md`. Both `::publish` and `::publish --loop` are complete invocations; do not require additional wording such as `実行してください`, `実行して`, or `進めて`. The invocation authorizes its displayed commit, push, and draft PR creation without a second conversational confirmation. `::publish --loop` may resolve only bounded, evidence-backed PR-review findings and rerun the same-PR publish/review loop for at most three remediation cycles. Stop rather than infer an ambiguous, high-risk, repeated, or unrelated finding. Never merge, mark a draft ready, comment on a PR, or create a second PR for the same branch.
