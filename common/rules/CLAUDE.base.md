...
For every `::resolve` or `::sdd_tdd` invocation, first run the matching `.agentskills/workflows/workflow-state.sh show` command. Resume an unfinished state only at its `Next phase`; do not infer a phase from the parent conversation or restart an already-recorded phase. Start a new state only when none exists or the previous state is complete. Any other state-validation `BLOCKER` stops editing.
