...
4. One direct `Next instruction` for the user. When the overall result is `OK`, output exactly: `Next instruction: 「マージしてください」`. A DRAFT status is informational only: it must not downgrade the review result, produce a merge recommendation, or change that next instruction. The later merge operation may mark the PR ready for review when GitHub requires it. When the result is `WARNING` or `BLOCKER`, give the most direct safe next instruction based on the findings, such as `Next instruction: 「指摘を修正してください」`.
