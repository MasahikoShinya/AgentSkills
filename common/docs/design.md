...
`--auto`を付けると、期待動作と対象範囲が明確な依頼に限り、SpecからGateまでをphaseごとの確認なしで連続実行する。`SESSION_BRIEF.md`はPhase 1で自動更新し、commit、push、mergeは行わない。仕様の曖昧さ、既存差分の混在、project-nativeなテスト手段の不明、test/review/gate/hookの`WARNING` / `BLOCKER` / `FAIL`、security・外部公開・不可逆操作では停止する。失敗時は`failure-analysis.md`による分析までを自動化し、同じrunで連続修正しない。
