...
開始後に`--auto`を追加する場合は `::sdd_tdd --auto --resume` を使う。state helperが`.git/agentskills/workflows/sdd_tdd.state`に記録した次Phase、開始時のstaged files、SESSION_BRIEF hashを確認し、整合する場合だけ再開する。
