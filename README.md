...
### Cowork 側に直接「現状のソースを反映して」と頼む場合

**Claude Code（または手動）で `git pull` 済み**であれば、Cowork on Mac / Windows に以下を貼るだけで、Cowork 自身が .skill をパッケージし、present_files で提示してくれる。**バージョンや特定スキル名に依存しないので、今後の更新でもそのまま流用可能。**
...
**他スキル（test-orchestrator など）にも流用したい場合：** 上記のプロンプトの `cowork-chrome-launcher` 部分を対象のスキル名に差し替えるだけで OK。バージョン情報やコミットハッシュを書いていないので、運用の中で同じテンプレートを使い続けられる。
